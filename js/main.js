$(document).ready(function(){
    ...
    var swiper = new Swiper(".mySwiper", {
      ...
      $('.tab_title ul li').click(function(){
        ...
        //모든 .tab_list 안보임
        $('.tab_list').hide();
        //idx 변수의 값과 같은 인덱스 번호에 해당하는 tab_list 만 보임
        $('.tab_list').eq(idx).fadeIn();
        return false; //클릭했을 때 위로 올라감 방지
      });
      //자동 탭메뉴
      var auto=setInterval(autoTab, 6000);
      //autoTab 함수 선언
      function autoTab(){
          //idx 변수 값을 1씩 증가시킴
          idx++;
    ...
    //window의 높이값을 winH 변수에 저장
    var winH=$(window).height();
    //window에  scroll 이벤트 설정 (header)
    $(window).scroll(function(){
      ...
      //만약 win 값이 window의 높이값보다 크면 top 버튼 나타나고, 작으면 top버튼 사라짐
      if(win>winH){
        $('.top').addClass('active');
      }else{
